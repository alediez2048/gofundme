...
**Date:** March 8, 2026
**Objective:** Answer 20 critical pre-code questions using first principles, competitive research (BuiltWith + Semrush), and an uncompromising focus on UX quality.
...
- **Performance:** TTFB ≤ 150ms via static generation. LCP ≤ 1.8s because above-the-fold content is in the initial HTML. Client-side transitions after first load are sub-100ms. This directly beats GoFundMe's measured 491ms TTFB and 5.2s full load.
...
**Why it matters for the fundraiser:** The donation conversion funnel starts the moment a page loads. Every 100ms of delay reduces conversion by 1-2%. A fundraiser page that loads in 1.8s instead of 5.2s isn't just faster — it's structurally more likely to receive donations. The hybrid approach means the first page load (typically from an external link or search result) is server-fast, and every subsequent navigation within the platform (community → fundraiser → profile) is instant.
...
**Why it matters for the fundraiser:** The Semrush audit revealed that GoFundMe's Community pages are "structurally thin and semantically generic." Their Profile pages are deindexed dead zones. AI fills both gaps — Cause Intelligence gives Community pages the content depth they need to rank for informational queries, and personalized recommendations give Profile pages a reason to exist beyond static credential display. The Story Generator addresses the cold-start problem: most fundraisers fail not because the cause is bad, but because the story is poorly written.
...
**Why it matters for the fundraiser:** Social proof drives donations. A fundraiser page showing "Sarah donated $100 2 hours ago" with a clickable link to Sarah's profile creates three things simultaneously: urgency (recent donation), trust (a real person with a history), and engagement (the profile click deepens the session). The Semrush audit showed GoFundMe's Profile pages are deindexed — meaning this social proof layer doesn't feed back into their search presence. Ours does.
...
# Interview Blueprint: Supplementary Questions 21–23

**Context:** These three questions close the gaps identified after the initial 20. They cover deployment, AI key management, and accessibility — decisions that are small in isolation but catastrophic if wrong on demo day.

---

## Part 9: Deployment & Demo-Day Readiness

---

### Question 21: Deployment Strategy — Where Does This Actually Run?

**The tension:** A Vercel deployment gives the evaluator a URL they can click immediately. A local-only setup requires them to clone, install, and run — friction that filters out busy reviewers. But a hosted deployment introduces environment variable management, cold starts, and the risk of the live site breaking after submission.

#### Option A: Vercel Deployment Only (Live URL)

Deploy to Vercel. Submit a URL. The evaluator clicks and sees the product instantly. No local setup required.

- **User Engagement:** Maximum first impression. Zero friction between "open the submission" and "see the product." The evaluator is interacting within 3 seconds.
- **Performance:** Vercel's edge network serves Next.js with optimal caching. SSG pages are served from CDN with near-zero TTFB. Serverless functions handle any API routes. Cold starts on serverless are 200-500ms but only affect the first request after inactivity.
- **Implementation Complexity:** Low. `vercel deploy` from the CLI. Environment variables configured in the Vercel dashboard. Automatic preview deployments on every git push. But there's a risk: if the evaluator tests 2 weeks after submission and a dependency has broken, the live site may be down.

#### Option B: Local-Only (Clone + npm install + npm run dev)

No deployment. Submit a repo link. The evaluator clones it and runs locally.

- **User Engagement:** High friction. The evaluator must have Node.js installed, run `npm install` (which can take 30-60 seconds), and start the dev server. If any dependency fails to install (version conflicts, platform-specific issues), the demo dies before it starts.
- **Performance:** Local dev server is unoptimized — no SSG, no edge caching, no CDN. The evaluator sees development performance, not production performance. This directly undermines the "fast response times" requirement because dev mode is measurably slower.
- **Implementation Complexity:** Lowest for you. But the burden shifts to the evaluator — and every minute of their setup time is a minute of goodwill you're spending.

#### Option C: Dual Deployment (Vercel Live URL + Local Setup That Actually Works)

Deploy to Vercel for the primary demo experience. Also ensure the local setup works flawlessly with a single command (`npm install && npm run dev`). Include a `.env.example` file with clear instructions. The README links to the live URL first, with local setup as a fallback.

- **User Engagement:** Best of both worlds. The evaluator clicks the live URL and sees the product in 3 seconds. If they want to inspect the code running locally, they can. If Vercel goes down, the local setup is a safety net.
- **Performance:** Live URL shows production performance (SSG, edge caching, optimized bundles). Local setup shows dev-mode performance — but the evaluator is now looking at code, not judging speed.
- **Implementation Complexity:** Medium. Requires maintaining both deployment paths. The `.env.example` file must be accurate. The `README` must clearly distinguish "click here to see the live demo" from "run locally to inspect the code."

#### Winner: Option C — Dual Deployment

**Why it matters for the fundraiser:** The evaluator's first 10 seconds determine their impression. If they click a URL and see a fast, polished product, you've already passed the "fast response times" requirement before they read a line of code. If they have to clone a repo and troubleshoot `npm install`, you've failed before they've seen anything. The live URL is the product. The local setup is the code review.

**How it will be implemented:**

- **Vercel setup:**
  - Connect the GitHub repo to Vercel. Enable automatic deployments on `main` branch.
  - Configure environment variables in Vercel dashboard: `NEXT_PUBLIC_AI_ENABLED=true`, `OPENAI_API_KEY=sk-...` (for build-time AI generation only — not exposed to the client).
  - Custom domain optional but not necessary. The `*.vercel.app` URL is sufficient.
  - Run `vercel --prod` before submission to ensure the production build is clean.

- **Local setup:**
  - `.env.example` file:
    ```
    # Optional: Enable AI features (Cause Intelligence generation)
    # Without this key, pre-generated content is used instead
    OPENAI_API_KEY=

    # Analytics (no configuration needed — runs in-memory)
  ...
  - `package.json` scripts:
    ...
    ```
  - Node.js version pinned in `.nvmrc`: `20` (LTS). `engines` field in `package.json` for enforcement.
  - Lock file committed (`package-lock.json`) to ensure deterministic installs.

- **README structure:**
  ```markdown
  ## Live Demo
  👉 [View the live application](https://fundright.vercel.app)
...
**Expected outcome:** The evaluator clicks the live URL and is interacting with the product in under 5 seconds. If they want to go deeper, the local setup works with two commands and zero configuration. Both paths lead to the same product. No single point of failure.

---

### Question 22: AI API Key Management — What Happens Without a Key?

**The tension:** The Cause Intelligence and Story Generator features require an LLM API call. If the evaluator runs locally without an API key, those features either break (bad), show an error (mediocre), or gracefully degrade to pre-generated content (good). This is a demo-day reliability question disguised as a configuration question.

#### Option A: Require the API Key (Fail Without It)

AI features require a valid `OPENAI_API_KEY` in `.env`. Without it, the features crash or show error states.

- **User Engagement:** Negative. An error on the Community page's Cause Intelligence section screams "broken." The evaluator doesn't know or care that they need an API key — they just see a failed feature.
- **Performance:** N/A when it works. Catastrophic when it doesn't.
- **Implementation Complexity:** Lowest. No fallback logic. `if (!key) throw new Error()`.

#### Option B: Environment Variable with Fallback to Placeholder Text

Check for the API key at runtime. If present, call the LLM. If absent, render a placeholder: "AI-generated content unavailable — set OPENAI_API_KEY to enable."

- **User Engagement:** Better than crashing, but the placeholder text tells the evaluator "this feature doesn't work in your environment." It's an excuse, not a solution. The section exists but adds nothing.
- **Performance:** No impact. The placeholder renders instantly.
- **Implementation Complexity:** Low. `if (key) { callAI() } else { renderPlaceholder() }`.

#### Option C: Build-Time AI Generation with Static Fallback

AI content (Cause Intelligence summaries) is generated at build time and baked into the static page. The Story Generator uses a pre-generated example as its default state, with live generation available if an API key is configured. The product works perfectly with zero API keys — AI content is simply pre-generated rather than live-generated.

- **User Engagement:** Maximum. The evaluator sees AI-generated content on every page regardless of their environment. The Community page has a rich Cause Intelligence section. The Story Generator shows a pre-written example that demonstrates what it produces. If the evaluator runs with an API key, the Story Generator is live. If not, the demo is still complete.
- **Performance:** Better than runtime AI calls. Build-time generation means zero API latency during page loads. The Cause Intelligence section is static HTML — fastest possible rendering.
- **Implementation Complexity:** Medium. Requires two code paths: (1) a build script that calls the LLM and writes output to seed data, and (2) a runtime check that enables live Story Generator if a key is present. The seed data ships with pre-generated AI content so the app never depends on a live API call.

#### Winner: Option C — Build-Time AI Generation with Static Fallback

**Why it matters for the fundraiser:** This is a reliability-first decision. The evaluator will not configure an API key. Assume they won't. Assume they'll clone the repo, run `npm run dev`, and expect everything to work. If your AI features are invisible without a key, you've effectively removed your biggest differentiator from the demo. Pre-generating AI content at build time means the product ships complete — the API key unlocks live generation as an enhancement, not a requirement.

**How it will be implemented:**

- **Build-time generation (your development environment):**
  ```typescript
  // scripts/generate-ai-content.ts
  // Run manually: npx tsx scripts/generate-ai-content.ts
  // Generates Cause Intelligence for each community and writes to seed data

  import { generateCauseIntelligence } from '../lib/ai/cause-intelligence';
  import { communities } from '../lib/data/seed';

  for (const community of communities) {
    const content = await generateCauseIntelligence(community.name, community.causeCategory);
    community.causeIntelligence = content;
  }

  // Write updated seed data to lib/data/seed.ts
  writeSeedFile(communities);
  ```

- **Seed data ships with pre-generated content:**
  ```typescript
  // lib/data/seed.ts (committed to repo)
  export const communities = {
    'watch-duty': {
      // ...other fields
      causeIntelligence: "Wildfire seasons have intensified dramatically over the past decade, with the 2025 season burning over 7.5 million acres across the western United States..."
      // This content was generated by Claude at build time and committed as static data
    }
  };
  ```

- **Story Generator with graceful degradation:**
  ```typescript
  // components/StoryGenerator.tsx
  const hasApiKey = !!process.env.OPENAI_API_KEY;

  // Default: show a pre-generated example story
  const [story, setStory] = useState(EXAMPLE_GENERATED_STORY);
  const [isLive, setIsLive] = useState(false);

  const generateStory = async (bullets: string[]) => {
    if (hasApiKey) {
      // Live generation via API route
      const response = await fetch('/api/generate-story', { method: 'POST', body: JSON.stringify({ bullets }) });
      const { story } = await response.json();
      setStory(story);
      setIsLive(true);
    } else {
      // Show pre-generated example with explanation
      setStory(EXAMPLE_GENERATED_STORY);
      setIsLive(false);
    }
  };
  ```
...
- **UI indicator:**
  - If live generation is available: "✨ AI-powered" badge on the Story Generator
  - If using pre-generated content: "📝 Example output — configure OPENAI_API_KEY for live generation" (subtle, non-blocking)

- **`.env.example` documentation:**
  ```
  # Optional: Enable live AI story generation
  # Without this key, pre-generated AI content is used (the app works fully without it)
  OPENAI_API_KEY=
  ```

**Expected outcome:** The product works perfectly with zero configuration. AI content is visible on every page. The Cause Intelligence section renders pre-generated summaries indistinguishable from live-generated ones. The Story Generator shows a compelling example. If the evaluator configures an API key, live generation activates as a bonus. The AI differentiator is never invisible.

---

### Question 23: Accessibility — Keyboard Navigation, Screen Readers, and ARIA

**The tension:** The QA checklist targets Lighthouse accessibility ≥ 90, but a score is not a strategy. Accessibility isn't a feature you add at the end — it's a set of design decisions baked in from the start. The questions so far cover responsive design (visual accessibility) but not keyboard navigation, screen reader support, focus management, or ARIA semantics.

#### Option A: Accessibility as Afterthought (Fix Lighthouse Warnings)

Build the product, run Lighthouse, fix whatever warnings appear. Add `alt` tags to images, `aria-label` to buttons, and call it done.

- **User Engagement:** Passes automated audits but fails real-world accessibility. A screen reader user encounters unlabeled interactive elements, broken focus traps in the donation modal, and no skip navigation link. The Lighthouse score is 90+ but the experience is unusable for assistive technology users.
- **Performance:** No impact.
- **Implementation Complexity:** Low — but only because it's incomplete. Fixing Lighthouse warnings is ~1 hour. Making the product actually accessible is ~5 hours.

#### Option B: Full WCAG 2.1 AA Compliance

Follow every WCAG 2.1 AA criterion: color contrast ratios, keyboard operability, focus indicators, ARIA landmarks, live regions for dynamic content, skip navigation, reduced motion support, and screen reader testing with VoiceOver/NVDA.

- **User Engagement:** Fully accessible. Any user, regardless of ability, can navigate and use the platform. But full compliance requires significant testing time and edge-case handling that may not be visible to the evaluator.
- **Performance:** Slightly positive — semantic HTML and proper heading hierarchy improve both accessibility and SEO (search engines parse semantic structure).
- **Implementation Complexity:** High. Full WCAG AA compliance is a substantial effort, especially for interactive elements like the donation modal and analytics dashboard charts.

#### Option C: Accessibility-First Foundation with Strategic ARIA

Build with semantic HTML from the start (proper heading hierarchy, landmark regions, button elements for interactive actions). Add focused accessibility investment in three high-impact areas: (1) keyboard navigation for the complete donation flow, (2) focus management for the donation modal, and (3) ARIA live regions for dynamic updates (progress bar changes, toast notifications). Skip the long tail of WCAG criteria that won't be tested.

- **User Engagement:** The donation flow — the most important interaction — is fully keyboard-accessible and screen-reader-friendly. The rest of the product uses semantic HTML, which provides a strong baseline. The evaluator who tabs through the donation flow sees proper focus indicators, logical tab order, and correct ARIA labels.
- **Performance:** Positive. Semantic HTML (`<nav>`, `<main>`, `<article>`, `<aside>`) replaces generic `<div>` soup. This improves both accessibility and SEO crawlability.
- **Implementation Complexity:** Medium. Semantic HTML is a one-time decision that costs nothing extra. The donation modal focus trap is ~30 lines. ARIA live regions are ~10 lines per dynamic element. The total investment is ~4 hours for a disproportionate impact.

#### Winner: Option C — Accessibility-First Foundation with Strategic ARIA

**Why it matters for the fundraiser:** Accessibility isn't just compliance — it's trust. A fundraising platform that excludes users with disabilities is a fundraising platform that excludes donors. More practically: semantic HTML improves SEO (the Semrush audit showed GoFundMe's schema is weak — semantic HTML gives crawlers more structure to work with), and keyboard accessibility catches power users who tab through forms instead of clicking. The donation modal is the highest-stakes interaction in the product — if it traps keyboard focus correctly, that's a signal of engineering quality that most submissions won't demonstrate.
...
- **Heading hierarchy (enforced per page):**
  - Fundraiser page: `<h1>` = fundraiser title, `<h2>` = "Our Story", "Recent Donors", "Updates"
  - Community page: `<h1>` = community name, `<h2>` = "About This Cause", "Active Fundraisers", "FAQ"
  - Profile page: `<h1>` = user display name, `<h2>` = "Impact", "Fundraisers", "Communities", "Giving History"
  - No heading level skips. No `<h1>` used for visual sizing (use CSS instead).

- **Donation modal focus trap:**
  ```typescript
  // hooks/useFocusTrap.ts
  export function useFocusTrap(modalRef: RefObject<HTMLElement>, isOpen: boolean) {
    useEffect(() => {
...
- **ARIA live regions for dynamic updates:**
  ```tsx
  // Progress bar announces changes to screen readers
  <div
    role="progressbar"
    aria-valuenow={raisedAmount}
    aria-valuemin={0}
    aria-valuemax={goalAmount}
    aria-label={`${Math.round((raisedAmount / goalAmount) * 100)}% of $${goalAmount} goal raised`}
  >
    <div className="h-full bg-emerald-600 transition-all duration-500" style={{ width: `${percentage}%` }} />
  </div>
  ...
  // Donor wall updates announce new donations
  <section aria-label="Recent donations" aria-live="polite">
...
- **Focus indicators (visible, not just default):**
  ```css
  /* globals.css */
  *:focus-visible {
...
- **Color contrast verification:**
  - Primary text (#1c1917) on background (#fafaf9): contrast ratio 15.3:1 (AAA pass)
  - CTA button text (white) on emerald-600 (#059669): contrast ratio 4.6:1 (AA pass)
  - Caption text (#78716c, stone-500) on background (#fafaf9): contrast ratio 4.5:1 (AA pass, borderline — may need to darken to stone-600)
...
- **Keyboard navigation testing checklist:**
  ```
  □ Tab through entire fundraiser page — logical order, no focus traps
  □ Tab to "Donate" button → Enter opens modal → focus moves to modal
  □ Tab within modal — cycles through amount input, impact preview, confirm button
  □ Escape closes modal → focus returns to "Donate" button
  □ Tab through community page — fundraiser cards are focusable and activatable with Enter
  □ Tab through profile page — all links and interactive elements reachable
  □ Skip nav link works — Tab from page load → Enter → focus jumps to main content
  □ Screen reader announces progress bar percentage on change
  □ Screen reader announces toast notifications
  ```

**Expected outcome:** Lighthouse accessibility score ≥ 95 (not just 90). The donation flow is fully keyboard-accessible — an evaluator who tabs through the entire donation process encounters proper focus management, logical tab order, and clear ARIA labels. Screen readers announce dynamic updates (progress bar changes, new donations, toast notifications). The product passes automated accessibility audits and performs well in manual keyboard testing. Total implementation cost: ~4 hours, integrated into the normal build process rather than bolted on at the end.

---

## Updated Decision Matrix (Questions 21–23)

| # | Question | Winner | Time Estimate |
|---|---|---|---|
| 21 | Deployment Strategy | Dual Deployment (Vercel + Local) | 2 hours |
| 22 | AI API Key Management | Build-Time Generation + Static Fallback | 3 hours |
| 23 | Accessibility | Accessibility-First Foundation + Strategic ARIA | 4 hours |

**Revised total estimated implementation time: ~64 hours (55 + 9)**

This adds less than a day of work to the original estimate but eliminates three categories of demo-day failure: deployment friction (the evaluator can't run it), missing AI content (the evaluator doesn't have an API key), and accessibility gaps (the evaluator tabs through the donation flow and hits a focus trap).

---

## Complete Decision Matrix (All 23 Questions)

| # | Question | Winner | Time Est. |
|---|---|---|---|
| 1 | SPA or Multi-Page? | Next.js Hybrid (SSR/SSG + Client Navigation) | — |
| 2 | Where Does AI Live? | Content Infrastructure (3 integrated features) | 8h |
| 3 | Real Backend or Mock? | In-Memory Reactive Data Layer (Zustand) | 4h |
| 4 | How Do Pages Connect? | Rich Graph with Prioritized Edges | 3h |
| 5 | Minimum Schema? | Demo-Complete (every visible field, nothing more) | 2h |
| 6 | What Is "Well Instrumented"? | Four-Tier Instrumentation | 6h |
| 7 | What Tooling? | Custom Analytics Layer (zero dependencies) | 4h |
| 8 | MVP Cut? | Polished Wedge (3 pages + dashboard + 1 AI feature) | — |
| 9 | Outside the Box? | Impact Projections + Structural SEO Innovation | 4h |
| 10 | Demonstrate Integration? | Shared State with Visual Feedback | 3h |
| 11 | Visual Identity? | Warm Modern (elevated GoFundMe) | 3h |
| 12 | Mobile or Desktop First? | Responsive-Concurrent | — |
| 13 | Emotional Arc? | Emotion-First with Progressive Data | — |
| 14 | Image Handling? | Curated Static + next/image | 2h |
| 15 | State Management? | Zustand with Normalized Slices | 3h |
| 16 | Routing & Transitions? | Skeleton Loaders + Prefetch + Subtle Fade | 2h |
| 17 | Funnel Definition? | Full Journey + Trust-Verification Branch | 2h |
| 18 | Surface Instrumentation? | Dedicated Analytics Dashboard | 4h |
| 19 | Edge Cases? | Designed Edge Cases | 3h |
| 20 | Testing & QA? | Targeted Tests + Pre-Submit Checklist | 2h |
| 21 | Deployment Strategy? | Dual Deployment (Vercel + Local) | 2h |
| 22 | AI API Key Management? | Build-Time Generation + Static Fallback | 3h |
| 23 | Accessibility? | Accessibility-First Foundation + Strategic ARIA | 4h |

**Total: ~64 hours across a 2-week sprint**
